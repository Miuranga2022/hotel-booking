...
const AllRooms = () => {
    ...
    //Filter and sort rooms based on the selected filters ad sort option
    const filteredRooms = useMemo(() => {
        return rooms
            .filter(room => matchesRoomType(room) && matchesPriceRange(room) && filterDestination(room))
            .sort(sortRooms);
    }, [rooms, selectedFilters, selectedSort, setSearchParams]);
        ...
        <div className="flex flex-col-reverse lg:flex-row items-start justify-between pt-28 md:pt-35 px-4 md:px-16 lg:px-24">
            <div>
                <div className="flex flex-col items-start text-left">
                    ...
                    <p className="text-sm md:text-base text-gray-500/90 mt-2 max-w-[696px]">Take advantage of our limited-time offers and spectial packages to enhance your stay and create unforgettable memories.</p>
                </div>
                ...
                {filteredRooms.map((room) => (
                    <div key={room._id} className="flex flex-col md:flex-row items-start py-10 gap-6 border-b border-gray-300 last:pb-32 last:border-0">
                        <img src={room.images[0]} alt="hotel-img" title="View Room Details" className="max-h-64 md:w-1/2 rounded-xl shadow-lg object-cover cursor-pointer" onClick={() => { navigate(`/rooms/${room._id}`); scrollTo(0, 0) }} />
            ...
            <div className="bg-white w-80 border border-gray-300 text-gray-600 max-lg:mb-8 min-lg:mt-16 rounded-md shadow-sm">
                <div className="flex items-center justify-between px-5 py-3 border-b border-gray-300">
                    ...
                    <span className="hidden lg:inline-block text-sm text-blue-600 cursor-pointer">
                        CLEAR
                    </span>
                </div>
                ...
                <div
                    className={`${openFilters ? "h-auto py-4" : "h-0 py-0"
                        } overflow-hidden transition-all duration-500 lg:h-auto lg:py-4`}
                >
                    <div className="px-5">
